...
export const CardRating = ({ gameSlug }) => {
  const { updateGameFromCollection, getGameFromCollection } = useCollection();
  ...
  useEffect(() => {
    getGameFromCollection(gameSlug).then((data) => {
      setGameData(data);
    });
  }, [gameSlug]);
