...
export const mySocialLinks = [
  ...
  },
  {
    ...
    name: "icon-[icon-park-solid--update-rotation]",
    url: "https://danielcabral.onrender.com/tags/gamenest",
  },
];
        ...
        <div className="space-y-2 sm:flex sm:items-center sm:justify-between sm:space-y-0">
          <ul className="flex items-center justify-center gap-2">
            {mySocialLinks.map((socialLink) => (
              <li
                key={socialLink.id}
                className="tooltip tooltip-top transition-transform duration-300 ease-in-out hover:scale-110"
                data-tip={socialLink.title}
              >
                <a href={socialLink.url} target="_blank" rel="noreferrer">
                  <Icon
                    name={socialLink.name}
                    className="size-8 text-gray-300 md:size-10 lg:size-11"
                  />
                </a>
              </li>
            ))}
